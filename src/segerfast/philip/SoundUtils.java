...
public class SoundUtils {
    ...
    public static void stopGameMenuMusic() {
        ...
        final float volume = ((FloatControl) menuMusicClip.getControl(FloatControl.Type.MASTER_GAIN)).getValue();

        Runnable decreaseVolume = () -> {
            try {
                for(float vol = volume; vol > -80; vol-=0.5) {
                    menuMusicVolumeControl.setValue(vol);
                    Thread.sleep(30);
                }
                menuMusicClip.stop();
